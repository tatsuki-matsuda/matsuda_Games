...
public:
	...
	BULLETTYPE GetBulletType(void);																//弾情報の呼び出し
	void SetBulletType(BULLETTYPE bulletType);													//弾情報の取得

	ATTACKTYPE GetAttackType(void);																//攻撃種類情報の呼び出し
	void SetAttackType(ATTACKTYPE attackType);													//攻撃種類情報の取得

	INFOTYPE GetInfoType(void);																	//攻撃情報の呼び出し
	void SetInfoType(INFOTYPE infoType);														//攻撃情報の取得

	bool GetGraze(void);																		//グレイズ情報の呼び出し
	void SetGraze(bool bgraze);																	//グレイズ情報の取得

	bool GetUninit(void);																		//弾消し情報の呼び出し
	void SetUninit(bool bUninit);																//弾消し情報の取得

	int  GetLife(void);																			//体力情報の呼び出し
	void SetLife(int nLife);																	//体力情報の取得
	
	D3DXVECTOR3  GetMove(void) { return m_move; }												//移動情報の呼び出し
	void SetMove(D3DXVECTOR3 move) { m_move = move; }											//移動情報の取得
