...
//マクロ定義
//--------------------------------------------------------------------------------
#define ENEMY_MINLIFE		(50)									//雑魚敵用最低基準値ライフ
#define ENEMY_LIFE			(100)									//雑魚敵用基準値ライフ
#define ENEMY_MAXLIFE		(300)									//雑魚敵用最大基準値ライフ

#define BOSS_MINLIFE		(1500)									//ボスの最低値ライフ
#define BOSS_LIFE			(2500)									//ボスのライフ
#define BOSS_LASTLIFE		(3200)									//ボスの最終ゲージライフ
...
public:
	...
	typedef enum
	{
		...
		ENEMYTYPE_SPIRAL,				//回転弾を撃つ敵
		ENEMYTYPE_REFLECTION_ALL,		//反射弾を撃つ敵
		ENEMYTYPE_REFLECTION_VERTICAL,	//上下反射弾を撃つ敵
	...
	{
		BOSSTYPE_NONE = 0,
		BOSSTYPE_CROSSING,				//交差弾を撃つボス
		BOSSTYPE_ILLUSION,				//幻覚弾を撃つボス
		BOSSTYPE_SPIRAL,				//回転弾を撃つボス
		BOSSTYPE_REFLECTION_ALL,		//反射弾を撃つボス
		BOSSTYPE_REFLECTION_VERTICAL,	//上下反射弾を撃つボス
		BOSSTYPE_TURN,					//折り返し弾を撃つボス
		BOSSTYPE_MAX
	...
	static CEnemy *Create(D3DXVECTOR3 pos, D3DXVECTOR3 scale,
		D3DXVECTOR3 move,ENEMYTYPE enemyType, BOSSTYPE bossType);				//敵の生成
	...
	ENEMYTYPE GetEnemyType(void) { return m_EnemyType; }						//敵情報の取得
	void SetEnemyType(ENEMYTYPE enemyType) { m_EnemyType = enemyType; }			//敵情報の呼び出し

	BOSSTYPE GetBossType(void) { return m_BossType; }							//敵情報の取得
	void SetBossType(BOSSTYPE bossType) { m_BossType = bossType; }				//敵情報の呼び出し

	ENEMYSTATE GetEnemyState(void) { return m_EnemyState; }						//状態情報の取得
	void SetEnemyState(ENEMYSTATE enemyState) { m_EnemyState = enemyState; }	//状態情報の呼び出し

	PHASETYPE GetFaseType(void) { return m_PhaseType; }							//フェーズ情報の取得
	void SetPhaseType(PHASETYPE phasetype) { m_PhaseType = phasetype; }			//フェーズ情報の呼び出し

	int GetLife(void) { return m_nLife; }										//体力情報の取得
	void SetLife(int nLife) { m_nLife = nLife; }								//体力情報の呼び出し

	int GetMaxLife(void) { return m_nMaxLife; }									//最大体力情報の取得
	void SetMaxLife(int nMaxlife) { m_nMaxLife = nMaxlife; }					//最大体力情報の呼び出し
...
private:
	...
	void OnMadnessBullet(void);													// 発狂の管理処理

	ENEMYTYPE m_EnemyType;														// 敵の種類
	BOSSTYPE m_BossType;														// ボスの種類
